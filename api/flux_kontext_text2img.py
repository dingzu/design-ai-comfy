...
class FluxKontextProNode:
    ...
    def generate_image(self, api_key, model, prompt, aspect_ratio, seed, prompt_upsampling, 
                      safety_tolerance, output_format, webhook_url="", webhook_secret=""):
        ...
        try:
            ...
            print(f"Sending request to FLUX Kontext {model.upper()} with prompt: {prompt[:50]}...")
            ...
            while attempt < max_attempts:
                ...
                try:
                    ...
                    if status == "Ready":
                        ...
                        success_msg = f"Image generated successfully with {model.upper()} model! Size: {image.size}"
                        print(success_msg)
            ...
            # Timeout
            error_msg = f"Generation timed out after {max_attempts} seconds"
            print(error_msg)
            return (torch.zeros((1, 512, 512, 3)), False, error_msg)
